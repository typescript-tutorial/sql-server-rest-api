...
export function query<T>(db: sql.ConnectionPool, sql: string, args?: any[], m?: StringMap, bools?: Attribute[]): Promise<T[]> {
  const request = db.request();
  const p = args ? args : [];
  if(args && args.length > 0) {
    p.forEach((item, i) => {
...
export function queryOne<T>(db: sql.ConnectionPool, sql: string, args?: any[], m?: StringMap, bools?: Attribute[]): Promise<T> {
  return query<T[]>(db, sql, args, m, bools)
    .then(results => results[0])
    .catch(err => err);
}
export function executeScalar<T>(db: sql.ConnectionPool, sql: string, args?: any[]): Promise<T> {
...
export function count(db: sql.ConnectionPool, sql: string, args?: any[]): Promise<number> {
